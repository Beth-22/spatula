<template>
  <div
    class="relative bg-white rounded-xl shadow-lg transition-all duration-500 cursor-pointer group overflow-hidden h-96 mx-auto"
  ...
  >
    ...
    <div
      class="absolute bottom-0 left-0 right-0 p-4 text-white z-10 transition-opacity duration-300"
    ...
    >
      <!-- Delete Button (Top-Right of Overlay) -->
      <button
        class="absolute top-3 right-3 w-10 h-10 bg-red-500/20 hover:bg-red-500/40 rounded-full flex items-center justify-center transition-colors duration-200"
...
import { Heart, Trash, Crown, ChefHat, Star, Timer, Eye } from "lucide-vue-next"
...
const recipeImage = computed(() => {
  return props.recipe.recipe_images?.[0]?.image_url || "https://images.unsplash.com/photo-1556909114-f6e7ad7d3136?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&h=300&q=80"
})

// Truncate description
const truncatedDescription = computed(() => {
  const desc = props.recipe.description || 'A delicious recipe waiting for you to explore.'
  const words = desc.split(' ')
  return words.length > 20 ? words.slice(0, 20).join(' ') + '...' : desc
})

// Navigation
const navigateToRecipeDetail = () => navigateTo(`/recipes/${props.recipe.id}`)
...
const handleLike = async (e) => {
  ...
  } catch (error) { console.error(error) } finally { likeLoading.value = false }
}

// Delete handler
const handleDelete = (e) => {
  e.stopPropagation()
  // emit or API call to delete
...
onMounted(async () => {
  ...
  if (isAuthenticated.value) {
    const status = await getLikeStatus(props.recipe.id)
